...
| Method | Path | Role |
|--------|------|------|
| GET | `/status` | Sync + session snapshot (statusline / internal) |
| POST | `/jam` | Kick sync, ensure monitor, `Session.play` |
| POST | `/sync` | Kick catalog sync |
| GET | `/devices` | List Connect devices + preferred id |
| POST | `/devices/{id}` | Save preferred device + transfer |
| POST | `/kill` | Stop monitor + uvicorn |
